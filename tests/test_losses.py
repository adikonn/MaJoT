...
# triangularization_loss
# ---------------------------------------------------------------------------
def test_tri_loss_zero_when_T_is_I_and_AB_upper_triangular():
    n = 5
    T = torch.eye(n)
...
def test_tri_loss_zero_for_ground_truth_Q_on_perfect_pair(n):
    A, B, Q = perfect_pair(n, dtype=torch.float64)
    assert float(triangularization_loss(Q, A, B)) == pytest.approx(0.0, abs=1e-8)


@pytest.mark.parametrize("n", [4, 8])
def test_ground_truth_Q_beats_identity_on_perfect_pair(n):
    A, B, Q = perfect_pair(n, dtype=torch.float64)
...
def test_tri_loss_batched(batch):
    n = 5
...
def test_tri_loss_non_negative():
...
def test_tri_loss_is_differentiable_wrt_T():
    n = 5
    A, B = random_pair(n)
...
# total_loss
# ---------------------------------------------------------------------------
def test_total_loss_returns_dict_components():
    n = 5
    A, B = random_pair(n)
    T = torch.eye(n)
...
def test_total_loss_components_match_total():
    n = 5
    A, B = random_pair(n)
    T = torch.randn(n, n)
...
def test_lambda_orth_zero_ignores_orth_term():
    n = 5
    A, B = random_pair(n)
    T = torch.randn(n, n)
...
def test_lambda_orth_scales_orth_term():
    n = 5
    A, B = random_pair(n)
    T = torch.randn(n, n)
    loss_1, _ = total_loss(T, A, B, lambda_orth=1.0)
    loss_2, _ = total_loss(T, A, B, lambda_orth=2.0)
    # Doubling lambda_orth should increase total by exactly one L_orth term.
