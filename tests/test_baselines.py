...
def test_returns_orthogonal_matrix(name, joint_triangularize, n) -> None:
...
@pytest.mark.parametrize(("name", "joint_triangularize"), BASELINES)
def test_minimizes_residual_vs_random(name, joint_triangularize) -> None:
...
@pytest.mark.parametrize(("name", "joint_triangularize"), BASELINES)
def test_cuda_support(name, joint_triangularize) -> None:
    n = 6
