...
```mermaid
flowchart TD
  A1["Реальные данные"\n"(сигналы, каналы, портфели, сенсоры)"] --> F1["Агрегация/фичи"\n"окна, условия, режимы"]
  F1 --> M1["Построение матриц"\n"A,B ∈ R^{n×n}"\n"(ковариации/моменты/операторы)"]

  subgraph S1["Совместное приведение пары матриц"]
    direction LR
    C1["Классика:"\n"QZ / generalized Schur"\n"(Jacobi-type и т.п.)"]
    N1["MaJoT (нейросеть):"\n"быстрый инференс T"]
  end
  ...
  R1 --> D1["Downstream-выгода"\n"(что реально важно бизнесу)"]
  D1 --> O1["Метрики продукта/системы"\n"качество, устойчивость,"\n"стоимость вычислений, latency"]

  subgraph U1["Примеры downstream по областям"]
    direction TB
    E1["EEG/ICA:"\n"стабильные компоненты"\n"→ классификация/BCI"]
    E2["MIMO:"\n"beamspace/сжатие CSI"\n"→ throughput/overhead"]
    E3["Common PCA:"\n"сопоставимость базиса"\n"между сессиями/условиями"]
    E4["Structural dynamics:"\n"быстрая редукция/оценка мод"\n"в design loop"]
  end
  ...
  subgraph W1["Почему нейросеть может быть выгоднее"]
    direction TB
    Wn1["1) Пакетность:"\n"много пар матриц в секунду"]
    Wn2["2) Шум/малая выборка:"\n"A,B оценены неточно"]
    Wn3["3) Специфичное распределение:"\n"можно обучить под задачу"]
    Wn4["4) End-to-end:"\n"дифференцируемый блок в пайплайне"]
  end
